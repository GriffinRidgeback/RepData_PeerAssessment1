...
The mean and median can be easily calculated from the daily totals, and the mean of all the steps over the two-month period is **`r prettyNum(mean(step_summary$daily_step_count))`** while the median is **`r median(step_summary$daily_step_count)`**.
...
And so the number of missing observations is the difference between the two, or **`r nrow(activity) - nrow(complete_days_only)`**.
...
Once again, the mean and median can be easily calculated from the daily totals, and the mean of all the steps over the two-month period is **`r prettyNum(mean(complete_data$daily_step_count))`** while the median is **`r prettyNum(median(complete_data$daily_step_count))`**.  The imputed data had a negligible affect on both the mean and the median calculated values.  The data did, however, produce a gap in recorded values for the range between 18000 and 20000.
